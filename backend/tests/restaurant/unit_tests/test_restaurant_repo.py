...
def test_add_menu_item_with_tags(restaurant_repo, sample_restaurant):
    restaurant_repo.create_restaurant(sample_restaurant)
    item = MenuItem(name="Veggie Burger", price=10.00, tags=["Vegan", "Healthy"])

    restaurant_repo.add_menu_item(sample_restaurant.id, item)

    stored_res = restaurant_repo.get_by_id(sample_restaurant.id)
    stored_item = stored_res["menu"][0]
    assert "Vegan" in stored_item["tags"]
    assert "Healthy" in stored_item["tags"]
