...
class MenuService:
    ...
    def add_menu_item(self, owner_id: int, restaurant_id: str, item_data: dict):
        # Fear2-FR4: Add new item
        restaurant = self.get_and_verify_owner(restaurant_id, owner_id)
    ...
    def edit_menu_item(self, owner_id: int, restaurant_id: str, item_id: str, updated_data: dict):
        ...
        success = self.repository.update_menu_item(restaurant_id, item_id, updated_item)
        if not success:
            raise EntityNotFoundError(f"Menu item {item_id} not found.")
        return True
    

    def remove_menu_item(self, owner_id: int, restaurant_id: str, item_id: str):
    ...
    def get_and_verify_owner(self, restaurant_id: str, owner_id: int):
        ...
        if not restaurant:
            raise EntityNotFoundError(f"Restaurant with ID {restaurant_id} not found.")
        
        if restaurant.get('owner_id') != owner_id:
            raise PermissionError("Access denied: You do not own this restaurant.")
        return restaurant
